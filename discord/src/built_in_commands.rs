...
impl BuiltInCommands {
    ...
    async fn help(conn: &DbContext, ctx: &Context, msg: &Message) {
        if let Some(guild_id) = msg.guild_id {
            if let Ok(commands) = CommandRepository::new(conn)
                .get_all_commands_in_guild(guild_id.into())
                .await
            {
...
Strikethrough commands are unavailable because they require to be in an nsfw channel.",
            ...
            Reply::from_str(msg, "Help is not available in DMs")
                .send(ctx)
                .await;
        }
    }
    ...
    async fn offer(conn: &DbContext, ctx: &Context, msg: &Message) {
        if let Some(guild_id) = msg.guild_id {
            if let Some(index) = msg.content.find(' ') {
                ...
                if let Ok(offered_command) = CommandRepository::new(conn)
                    .get_command_from_name_and_guild(offer_name, guild_id.into())
                    .await
                {
